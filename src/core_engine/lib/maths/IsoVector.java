...
public class IsoVector {
	...
	public IsoVector toScreen(Coordinator coord, Camera c) {
		...
		switch (space) {
			case CANVAS:
				screen = coord.canvasToScreen(c, this);
				break;
			case WORLD:
				screen = coord.canvasToScreen(c, coord.worldToCanvas(this));
				break;
			default:
				screen = this.copy();
		}
	...
	public IsoVector toCanvas(Coordinator coord, Camera... c) {
		...
		if (space == CANVAS)
			...
			canvas = coord.worldToCanvas(this);
		else
			canvas = coord.screenToCanvas(c[0], this);
	...
	public IsoVector toWorld(Coordinator coord, Camera c) {
		...
		if (space == CANVAS)
			world = coord.canvasToWorld(this);
		else if (space == WORLD)
			world = this.copy();
		else
			world = coord.canvasToWorld(coord.screenToCanvas(c, this));
	...
	public IsoVector toWorldExact(Coordinator coord, Camera... c) {
		...
		if (space == CANVAS)
			world = coord.canvasToWorldExact(this);
		else if (space == WORLD)
			world = this.copy();
		else
			world = coord.canvasToWorldExact(coord.screenToCanvas(c[0], this));
